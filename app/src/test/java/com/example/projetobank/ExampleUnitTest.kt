...
class ExampleUnitTest {
    ...
    @Test fun validarSenhaLetraMaiuscula() {
        val usuario = Usuario("frann", "testeA")
        assertTrue(usuario.ehValido {  })
    }

    @Test fun validarSenhCaracterEspecial() {
        val usuario = Usuario("frann", "teste@")
        assertTrue(usuario.ehValido {  })
    }
    @Test fun validarSenhaAlphanumerica() {
        val usuario = Usuario("frann", "teste3")
        assertTrue(usuario.ehValido {  })
    }

    @Test fun validarSenhaAlphanumericaComLetraMaiscula() {
        val usuario = Usuario("frann", "tes3Te")
        assertTrue(usuario.ehValido {  })
    }

    @Test fun validarSenhaAlphanumericaComCaracterEspecial() {
        val usuario = Usuario("fran", "3@")
        assertTrue(usuario.ehValido {  })
    }

    @Test fun validarSenhaMaisculaComCaracterEspecial() {
        val usuario = Usuario("frann@gmail.com", "M@")
        assertTrue(usuario.ehValido {  })
    }
    @Test fun validarSenhaMaisculaComCaracterEspecialEAlphanumerica() {
        val usuario = Usuario("frannzita", "M@3")
        assertTrue(usuario.ehValido {  })
    }

    @Test fun validarUsuarioCampoVazio() {
